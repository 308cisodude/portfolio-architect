...
A successful BPD response replaces an earlier failure state. DKB CSV acquisition is active through the experimental auto-starting DKB Gateway in v1.45.0; authenticated FinTS remains disabled until later authenticated user-capability gates are separately implemented and accepted.
