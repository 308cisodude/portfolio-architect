...
v1.62.0 adds health schema 10 `provider_name` and Supervisor discovery transport schema 2 for exact per-profile paths. One Generic App may expose up to eight isolated logical providers while Portfolio Architect continues to consume one canonical snapshot per provider; holdings/cash are never merged inside the Gateway. Existing `generic_csv` state retains its identity for compatibility. v1.62.1 changes ownership of first-run lifecycle: the Portfolio Architect integration must be initialized first, while ready Gateway discoveries are retained as source candidates for that existing singleton service. v1.62.2 leaves that transport unchanged and only hardens explicit first-run choices plus the Generic READY-profile colour presentation. v1.62.3 leaves those contracts unchanged and makes the Trade Republic cash as-of parser accept the complete bounded German abbreviated month-label matrix (including the live-observed `Sept.` form) with clearer missing-versus-ambiguous date errors. v1.62.4 is integration-only at runtime: it fixes first-run reload/unload lifecycle handling and moves private-CA trust loading off the Home Assistant event loop without changing Gateway acquisition, wire or trust contracts. v1.62.5 is likewise integration-only at runtime: it corrects optional `exceptions.yaml` participation in coordinator metadata/LKG fingerprints and leaves Gateway acquisition, wire and trust contracts unchanged. v1.63.0 is also Gateway-runtime neutral: only static dashboard authoring, localization, generation and presentation change; all provider acquisition, wire, authority and trust contracts remain unchanged.
