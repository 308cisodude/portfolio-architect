...
as a workaround for a validation/connection DNS race. From v1.27.2, official Supervisor Gateway Apps require verified HTTPS even on the private App network; legacy HTTP remains only as a bounded in-place migration state.
...
## v1.26.0 multi-Gateway aggregation boundary

Additional Gateway REST credentials are stored only in Home Assistant config-entry options and are never copied into payloads, dashboard attributes, source provenance, diagnostics, logs, or release artifacts. Each endpoint is constrained by the existing local-only REST transport protections, including private/loopback address validation, DNS pinning, no redirects, no proxy inheritance, no cookies, bounded responses, and bearer authentication.

Before an additional Gateway can join the aggregate, Portfolio Architect requires health schema 6, a bounded provider ID, a healthy/live snapshot, and matching timestamp/count/fingerprint evidence. Provider IDs are operational provenance only; account/depot/customer identifiers are neither requested nor accepted as provider identity. Duplicate endpoint/provider identities are rejected.
...
## v1.27.2 private-PKI HTTPS trust boundary
...
The v1.27.2 config-flow fix does not weaken the single-entry security boundary. The
manifest-level `single_config_entry` switch is removed only because Home Assistant
applies it before source-specific discovery handling and therefore blocked the
trusted `hassio` migration flow itself. Portfolio Architect explicitly refuses manual
setup when any domain entry already exists and retains its stable unique-ID guard.
Supervisor discovery remains constrained by exact source/provider/network identity,
private-CA verification and bearer authentication before any stored transport trust
changes.
