# Portfolio Architect Gateway — DKB v1.39.0

Version 1.39.0 is package alignment for Portfolio Architect's Home Assistant-side dynamic colourful current/target allocation presentation. The live-accepted v1.38.1 signed drift and v1.38.0 cash/ISIN work remain Home Assistant-side, and the v1.37 shared human-input helper remains present but unused by the exact FinTS registration/probe fields. DKB remains experimental, manual-only and non-live; the registered capability probe is unchanged.
...
The v1.39.0 DKB App requests no DKB login name, PIN or TAN and sends no holdings, balance,
