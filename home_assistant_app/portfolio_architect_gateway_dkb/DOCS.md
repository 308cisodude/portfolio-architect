# Portfolio Architect Gateway — DKB v1.38.1

Version 1.38.1 is package alignment for Portfolio Architect's Home Assistant-side native dynamic allocation-drift presentation. The v1.38.0 cash/ISIN dashboard work remains Home Assistant-side, and the v1.37 shared human-input helper remains present but unused by the exact FinTS registration/probe fields. DKB remains experimental, manual-only and non-live; the registered capability probe is unchanged.
...
The v1.38.1 DKB App requests no DKB login name, PIN or TAN and sends no holdings, balance,
