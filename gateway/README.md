# Portfolio Architect Gateway runtime v1.45.1

Version 1.45.1 hardens the v1.45 DKB migration bridge. The common runtime keeps normal cached-snapshot age enforcement unchanged, makes expired health documents schema-consistent, and adds an opt-in read-only migration-snapshot endpoint that is disabled by default and enabled only by the DKB App.
