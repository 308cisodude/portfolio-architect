"""v1.55.0 Generic Import Gateway and acquisition-boundary contracts."""
...
def test_release_versions_and_schema_are_aligned() -> None:
    ...
    for slug in (
        ...
        assert yaml.safe_load((APPS / slug / "config.yaml").read_text())["version"] == "1.55.0"
...
def test_schema_12_local_csv_migration_is_explicit_and_fail_closed() -> None:
    ...
    assert "Portfolio Architect Gateway — Generic Import v1.55.0" in schema12
