"""Regression contracts for v1.34.1 Gateway cold-restart snapshot integrity."""
...
def test_release_version_and_wire_contracts_remain_aligned() -> None:
    ...
    assert manifest["version"] == "1.34.1"
    ...
    assert "Gateway health schema 6: unchanged" in notes
...
def test_upgrade_guide_preserves_authentication_and_wire_compatibility() -> None:
    guide = (ROOT / "docs" / "UPGRADE-1.34.1.md").read_text(encoding="utf-8")
    assert "does not change portfolio strategy" in guide
    assert "No plan/schema migration is introduced by this hotfix" in guide
    assert "Do not reauthenticate Comdirect" in guide
    assert "REST portfolio schema 1 unchanged" in guide
    assert "Gateway health schema 6 unchanged" in guide
