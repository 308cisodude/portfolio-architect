...
def test_v1201_version_alignment_and_wire_compatibility() -> None:
    ...
    assert manifest["version"] == "1.27.2"
    assert app["version"] == "1.27.2"
    assert 'VERSION: Final = "1.27.2"' in const
    assert '__version__ = "1.27.2"' in engine
    assert '__version__ = "1.27.2"' in gateway
