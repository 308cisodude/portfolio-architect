...
def test_v1201_version_alignment_and_wire_compatibility() -> None:
    ...
    assert manifest["version"] == "1.20.1"
    assert app["version"] == "1.20.1"
    assert 'VERSION: Final = "1.20.1"' in const
    assert '__version__ = "1.20.1"' in engine
    assert '__version__ = "1.20.1"' in gateway
    ...
    assert '"requested_health_schema_version": 5' in rest_client
