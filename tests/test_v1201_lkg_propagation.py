...
def test_v1201_version_alignment_and_wire_compatibility() -> None:
    ...
    assert manifest["version"] == "1.26.4"
    assert app["version"] == "1.26.4"
    assert 'VERSION: Final = "1.26.4"' in const
    assert '__version__ = "1.26.4"' in engine
    assert '__version__ = "1.26.4"' in gateway
    ...
    assert "schema version 8" in integration_init
