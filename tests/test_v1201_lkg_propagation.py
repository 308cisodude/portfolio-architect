...
def test_v1201_version_alignment_and_wire_compatibility() -> None:
    ...
    assert manifest["version"] == "1.28.2"
    assert app["version"] == "1.28.2"
    assert 'VERSION: Final = "1.28.2"' in const
    assert '__version__ = "1.28.2"' in engine
    assert '__version__ = "1.28.2"' in gateway
    ...
    assert "schema version 9" in integration_init
    assert '"requested_health_schema_version": 6' in rest_client
