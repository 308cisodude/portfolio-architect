...
def test_current_version_metadata_is_aligned() -> None:
    assert 'version = "1.64.3"' in (ROOT / "pyproject.toml").read_text()
    assert '"version": "1.64.3"' in (COMPONENT / "manifest.json").read_text()
    assert 'VERSION: Final = "1.64.3"' in (COMPONENT / "const.py").read_text()
    assert '__version__ = "1.64.3"' in (COMPONENT / "engine" / "__init__.py").read_text()
    for app in (
