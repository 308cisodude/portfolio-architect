...
def test_v1210_version_and_wire_contracts_are_aligned() -> None:
    ...
    assert 'VERSION: Final = "1.57.0"' in (COMPONENT / "const.py").read_text(encoding="utf-8")
    assert '__version__ = "1.57.0"' in (COMPONENT / "engine" / "__init__.py").read_text(encoding="utf-8")
    ...
    assert '"health_schema_version": min(version, 8)' in gateway_server
