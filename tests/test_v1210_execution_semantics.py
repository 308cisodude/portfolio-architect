...
def test_v1210_version_and_wire_contracts_are_aligned() -> None:
    ...
    assert app["version"] == "1.58.0"
    assert 'VERSION: Final = "1.58.0"' in (COMPONENT / "const.py").read_text(encoding="utf-8")
    assert '__version__ = "1.58.0"' in (COMPONENT / "engine" / "__init__.py").read_text(encoding="utf-8")
