"""v1.64.4 UX, discovery-lifecycle, and presentation hygiene contracts."""
...
def test_v156_version_alignment_and_security_non_goals() -> None:
    ...
    assert manifest["version"] == "1.64.4"
    ...
    for slug in (
        ...
        assert yaml.safe_load((APPS / slug / "config.yaml").read_text())["version"] == "1.64.4"
...
def test_sbom_records_dkb_timezone_runtime_dependency() -> None:
    ...
    assert sbom["name"] == "Portfolio Architect v1.64.4 SBOM"
