...
def test_component_versions_are_compatible() -> None:
    ...
    assert manifest["version"] == "1.19.1"
    assert 'VERSION: Final = "1.19.1"' in const
    assert '__version__ = "1.19.1"' in engine
    assert app["version"] == "1.19.1"
    assert '__version__ = "1.19.1"' in gateway
