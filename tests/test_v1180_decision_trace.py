...
def test_v1180_metadata_and_compatibility_contracts_are_aligned() -> None:
    ...
    assert 'VERSION: Final = "1.22.0"' in (COMPONENT / "const.py").read_text()
    assert '__version__ = "1.22.0"' in (COMPONENT / "engine" / "__init__.py").read_text()
    ...
    assert "following milestone" in release_notes.lower()
