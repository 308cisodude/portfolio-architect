...
def test_v1530_versions_and_health_schema_are_aligned() -> None:
    assert json.loads((COMPONENT / "manifest.json").read_text())["version"] == "1.64.4"
    assert 'VERSION: Final = "1.64.4"' in (COMPONENT / "const.py").read_text()
    assert '__version__ = "1.64.4"' in (ROOT / "gateway/src/portfolio_architect_gateway/__init__.py").read_text()
    for app in (
