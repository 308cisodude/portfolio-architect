"""v1.52.0 capability-scoped Gateway maturity contracts."""
...
COMDIRECT = APPS / "portfolio_architect_gateway"
...
def test_current_release_versions_and_app_maturity_are_aligned() -> None:
    ...
    for app in (COMDIRECT, DKB, TR, GENERIC):
        assert _config(app)["version"] == "1.52.0"
...
def test_trade_republic_stable_label_does_not_invent_live_api_acquisition() -> None:
    ...
    assert "active imported Trade Republic Gateway snapshot" in en
    assert "aktiven importierten Trade-Republic-Gateway-Snapshots" in de
...
def test_generic_import_remains_experimental_and_live_smoke_is_isolated() -> None:
    ...
    guide = (ROOT / "docs" / "UPGRADE-1.52.0.md").read_text(encoding="utf-8")
...
def test_sbom_describes_all_four_gateway_apps() -> None:
    ...
    assert generic["versionInfo"] == "1.52.0"
