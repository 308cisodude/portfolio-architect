...
Version 1.35.2 completes the provider-scoped funding line with native execution-policy
configuration and a third provider-owned cash-authorization mode. Home Assistant can edit validated
broker schema-2/3 provider evidence, savings-plan routes and exact directed funding edges without
manual YAML; cost remains the primary route criterion, promotional status is descriptive only, and
provider preference remains an optional tie-break. Comdirect additionally supports **Keep cash
reserve**, authorizing only eligible cash above a retained EUR amount. The v1.35.1 session-maintenance
resilience fix remains unchanged.
...
Official v1.35.2 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
