...
Version 1.37.0 adds shared bounded Gateway parsing primitives for opt-in human numeric fields (EUR/money, percentages, quantities and bounded integers) and migrates the existing Comdirect cash-policy amount fields onto the shared EUR primitive without changing their live-proven v1.35.4 behavior. Protocol identifiers, registrations, credentials, tokens and exact IDs remain on provider-specific validation paths and are not locale-normalized.
...
Official v1.37.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
