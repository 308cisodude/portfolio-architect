...
Version 1.45.1 hardens the v1.45.0 DKB CSV migration bridge. An exact legacy comparison may now use a DKB-only authenticated migration snapshot even when that old canonical snapshot is beyond the Gateway's normal serving-age limit; the normal portfolio endpoint remains fail-closed. Expired Gateway health documents are also schema-consistent. Authenticated DKB FinTS remains gated and separate.
