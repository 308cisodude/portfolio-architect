...
Version 1.35.3 is a narrow presentation hotfix for the v1.35.2 native execution-policy editor.
Home Assistant now renders visible English/German labels for every Execution providers & funding
menu and nested Add/Edit/Remove action instead of unlabeled chevrons. Broker editing, cost-first
routing, tie-break/promotional semantics, Keep cash reserve authorization and the v1.35.1
Comdirect session-maintenance resilience fix remain unchanged.
...
Official v1.35.3 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
