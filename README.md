...
Version 1.46.0 retires the now-completed Home Assistant-side DKB CSV migration bridge. DKB depot CSV acquisition lives only in the DKB Gateway; the temporary legacy parser, source-path UI and migration-only endpoint are gone. Authenticated DKB FinTS remains gated and separate.
