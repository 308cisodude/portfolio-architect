...
Version 1.31.2 hardens the experimental DKB anonymous FinTS capability probe after the
first live registered attempt. It requires the complete 25-character product registration
identity, keeps Ingress POST navigation inside the DKB App, and persists sanitized probe
outcomes instead of reverting a failed attempt to `ready / not probed`. Valid `HIRMG`/`HIRMS`
return codes and bounded sanitized bank messages may remain as operator evidence; arbitrary
segment payload and the raw FinTS response are discarded after a decoded-response SHA-256
and byte count are recorded for correlation.
The v1.31 Robotics migration and v1.31.1 ISIN-only holding fix remain unchanged.
...
Official v1.31.2 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
