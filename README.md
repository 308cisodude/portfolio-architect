...
Version 1.32.0 adds per-source freshness evidence and explicit stale-source/actionability
blocker summaries while preserving the existing oldest-source fail-closed rule. It also
formalizes provider-specific diagnostic hardening: Trade Republic keeps only the latest
allowlisted private import outcome, Comdirect's existing bounded authenticated error model is
regression-protected, and the live-accepted v1.31.2 DKB anonymous FinTS diagnostics remain
unchanged. The reference dashboard uses native Tile attributes to show the actual blocker.
...
Official v1.32.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
