...
Version 1.36.0 completes native dynamic portfolio presentation. The reference dashboard now consumes bounded generic presentation-slot adapters and native Home Assistant `entity-filter` cards so configured targets, outside-scope holdings and active policy findings render without instrument-specific YAML inventories. Stable target/holding identities, provider behavior, execution/funding semantics and all Gateway wire/security contracts remain unchanged.
...
Official v1.36.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
