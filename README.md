...
Version 1.41.0 adds a separate strict Trade Republic `KONTOAUSZUG` cash-statement importer beside the established `DEPOTAUSZUG` holdings importer. Holdings and cash evidence remain independent, only bounded normalized cash state persists, and provider cash is freshness-gated separately before it can affect advisory funding decisions. No unofficial Trade Republic API, credential sharing, transaction-history persistence, trading or money-movement capability is introduced.
...
Official v1.41.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
