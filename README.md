...
Version 1.36.1 fixes the live-observed native-dashboard composition edge case from v1.36.0: dynamic `entity-filter` selection remains intact, while allocation inventories now feed core Entities cards instead of an empty nested Distribution card. Dynamic candidates also request entity-only display names so instrument labels stay compact without restoring hard-coded inventory.
...
Official v1.36.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
