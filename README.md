...
Version 1.43.0 adds route-level savings-plan evidence provenance/freshness and native editing for existing directed funding edges. Existing routes without route-level evidence continue to inherit provider-level evidence, while explicit provider/ISIN evidence can expire independently. The v1.42 execution-path presentation and all provider acquisition remain unchanged.
...
Official v1.43.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
