...
Version 1.38.0 adds Home Assistant-native dashboard usability polish: recommendation rows open a copy-friendly ISIN entity on tap while preserving purchase-explanation access on hold, and investment-cash Tiles now show total available cash plus the amount excluded by the active cash policy. Provider Gateway runtime behavior is unchanged. The v1.37.0 shared bounded human-numeric validation foundation remains in place for explicit opt-in fields.
...
Official v1.38.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
