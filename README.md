...
Version 1.39.0 adds paired colourful native current/target allocation Tiles for all 32 bounded generic target presentation slots. Corresponding current and target Tiles share deterministic slot identity colours and native 0–100% bar gauges, while the live-accepted v1.38.1 amber/green/red signed drift presentation remains unchanged. Provider Gateway runtime behavior is unchanged.
...
Official v1.39.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
