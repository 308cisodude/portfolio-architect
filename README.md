...
Version 1.26.2 retains the v1.26.1 ISIN-first multi-Gateway baseline and adds presentation/diagnostic polish only: the German reference dashboard uses explicit German presentation attributes, unavailable-source tiles identify privacy-safe source instances, and supplemental-source attention reasons no longer render as `None`. Provider acquisition and wire schemas are unchanged.
