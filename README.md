...
Version 1.44.0 is a native Configure UX consistency release. It audits every Configure menu, restores a regression-protected visible Edit funding transfer action, and adds non-editable identity context above every selected-object edit form. Planner economics, v1.43 route-evidence semantics, the v1.42 execution-path presentation, and all provider acquisition remain unchanged.
...
Official v1.44.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
