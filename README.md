...
Version 1.31.1 is a narrow Home Assistant-side identity-validation hotfix for the
v1.31 canonical Robotics migration. Trade Republic positions are legitimately ISIN-only;
when the former distributing Robotics holding became outside current plan scope, its empty
WKN must remain valid because `IE00BYWZ0333` supplies the canonical identity. The active
Robotics target remains `IE00BYZK4552` / `A2ANH0`, the historical exception remains
`superseded`, and provider-aware routing remains explicit configuration. No Gateway
acquisition, trading, or order-placement capability changes.
...
Official v1.31.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
