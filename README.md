...
Version 1.26.6 fixes the remaining unavailable-source diagnostic edge case found during v1.26.5 live acceptance: a reachable primary Gateway in `reauthentication_required` / Gateway-local LKG operation is now identified as `Comdirect Gateway` instead of yielding `Source unavailable: None`. No calculation, provider acquisition, authentication, source-trust, authorized-cash, date-presentation, or wire contract changes.
