...
Version 1.40.0 adds evidence provenance and freshness-aware fail-closed use of exact directed broker-schema-3 funding-transfer edges. New edges created through the native broker editor record verified fee, conservative settlement days, evidence source and evidence date; stale evidenced edges become ineligible for route selection. Provider Gateway runtime behavior and the live-accepted v1.39.0 dashboard presentation are unchanged.
...
Official v1.40.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
