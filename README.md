...
- Provider-aware Gateway health schema 7 with bounded provider identity plus explicit acquisition mode and backward-compatible health negotiation through schemas 1–6.
...
Version 1.52.0 aligns Gateway maturity with live evidence: Comdirect remains stable; DKB and Trade Republic graduate to stable for their established CSV/PDF acquisition paths; Generic Import remains experimental pending deliberate live exercise. The DKB anonymous FinTS probe remains explicitly experimental/research-only inside the otherwise-stable DKB App, and authenticated FinTS remains disabled. Provider acquisition, freshness, planner economics and wire/security contracts are unchanged.
...
- v1.52.0 App maturity: Comdirect, DKB and Trade Republic are stable for their supported acquisition paths; Generic Import remains experimental; the DKB anonymous FinTS probe remains experimental/research-only and authenticated FinTS remains unsupported.
