...
Version 1.28.1 is a release-engineering maintenance update. It refreshes every
Portfolio Architect `actions/checkout` workflow invocation to immutable v7.0.1 and
the validation/release `actions/setup-python` invocation to immutable v7.0.0,
removing the remaining Node.js 20 action runtime dependency. The v1.28.0 DKB FinTS
capability-probe boundary, Comdirect's live-proven v1.27.4 OAuth maintenance, Trade
Republic statement import, provider acquisition, schemas, calculations and Gateway
...
Official v1.28.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
