...
Version 1.38.1 restores native dynamic signed allocation-drift visualization for all 32 bounded generic target presentation slots using only core Home Assistant Conditional and Tile cards with the Tile-native `bar-gauge` feature. The v1.38.0 policy-aware cash context and copy-friendly recommendation ISIN interaction remain intact, and provider Gateway runtime behavior is unchanged. The v1.37.0 shared bounded human-numeric validation foundation remains in place for explicit opt-in fields.
...
Official v1.38.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
