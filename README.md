...
Version 1.35.4 is a narrow Comdirect Ingress hotfix for the v1.35.2 retained-cash authorization
feature. Human EUR amount fields now accept common decimal-comma/decimal-point and validated
thousands-grouping styles, normalize them to canonical private state, and return bounded validation
guidance instead of a generic browser HTTP 400 for invalid amount syntax. Broker editing, route
economics, cash-policy mathematics and the v1.35.1 session-maintenance resilience fix remain unchanged.
...
Official v1.35.4 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
