...
Version 1.26.4 is a final v1.26 dashboard-formatting cleanup. Date-only reference tiles now ask Home Assistant to render the native date state with its locale-aware short-date formatter instead of displaying raw ISO dates. No calculation, entity, provider, acquisition, availability, or wire contract changes.
