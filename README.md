...
Version 1.26.5 corrects the remaining date-only dashboard rendering issue found during v1.26.4 live acceptance. The established `SensorDeviceClass.DATE` sensors remain authoritative, while additive read-only native `date.*` presentation counterparts let Home Assistant apply its own locale-aware date-domain formatting. No calculation, provider acquisition, source trust, authorized-cash, or wire contract changes.
