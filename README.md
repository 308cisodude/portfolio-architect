...
Version 1.41.1 is a narrow provider-scoped funding hotfix: when local execution-provider cash and a cross-provider funding transfer are otherwise identical in cost, settlement, configured provider priority, order amount and fees, Portfolio Architect now prefers the local-cash route instead of falling through to lexical provider identifiers. The v1.41.0 Trade Republic `KONTOAUSZUG` cash importer and all provider/security boundaries remain unchanged.
...
Official v1.41.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
