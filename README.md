...
Version 1.34.0 makes target architecture explicitly generic: portfolio schema 2 uses opaque
PA-generated 128-bit `target_id` values that are independent from ISIN, WKN, name, order, and
weight. Existing schema-1 IDs remain supported. New targets created through the native editor
receive a fresh identity once; deleted targets are not tombstoned or resurrected from a later
matching instrument. A first-class structural presentation model exposes the current target and
holding inventories without introducing a custom frontend. Provider Gateway acquisition and
wire/diagnostic contracts remain unchanged apart from normal package/User-Agent alignment.
...
Official v1.34.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
