...
Version 1.40.1 is a Configure-menu compatibility and validation hotfix for v1.40.0. It fixes the Home Assistant 2026.8.1 savings-plan selector HTTP 400, moves broker evidence dates to native date selectors with Home Assistant-local current-date defaults, and adds bounded duplicate-object feedback. v1.40.0 evidence-backed funding semantics, provider Gateway runtime behavior and the live-accepted v1.39.0 dashboard presentation are unchanged.
...
Official v1.40.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
