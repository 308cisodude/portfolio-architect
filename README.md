...
v1.35.0 provider-scoped funding release. Direct connection-reset failures are classified as
retryable bounded transport errors, and the long-lived OAuth maintenance worker contains an
unexpected single iteration failure instead of terminating. Provider-scoped funding, broker
schema 3, REST portfolio schema 1, Gateway health schema 6 and the advisory/no-transfer boundary
remain unchanged.
...
Official v1.35.1 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
