...
Version 1.42.0 adds a bounded Home Assistant execution-path presentation contract for the already-decided actionable plan. The integration exposes ordered local-cash, advisory funding-transfer and purchase steps plus English/German rendering, while the native reference dashboard merely displays that integration-owned text and never reruns route or funding selection.
...
Official v1.42.0 Gateway Apps use verified HTTPS on the private Home Assistant App network and retain bearer authentication. Never expose the Gateway REST port to an untrusted network.
