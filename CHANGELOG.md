...
- Replace duplicated English/German dashboard authoring with one shared static Lovelace source, 100-key locale catalogs and bounded locale-specific technical overlays; Home Assistant receives generated ordinary YAML only and runs no localization/build code.
- Publish separate English-only and German-only dashboard artifacts alongside the existing combined EN/DE compatibility artifact, so single-language installations do not load a second full view and future locales do not multiply card logic in source.
